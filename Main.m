...
while 1 % Wait for ESC key (ASCII code 27).
    ...
    if dxl_direction==0 %방향 바뀌는 것에 따라 달라짐
        SetAngle(port_num,lib_name,PROTOCOL_VERSION,DXL_ID,ADDR_MOVING_SPEED,ADDR_MX_GOAL_POSITION,1500+anglecount*50, 200);
    elseif dxl_direction==1
        SetAngle(port_num,lib_name,PROTOCOL_VERSION,DXL_ID,ADDR_MOVING_SPEED,ADDR_MX_GOAL_POSITION,2500-anglecount*50, 200);
    end
    ...
    if anglecount==20 && dxl_direction==0 %방향 바꾸는 조건문
        anglecount=0; dxl_direction=1;
    elseif anglecount==20 && dxl_direction==1
